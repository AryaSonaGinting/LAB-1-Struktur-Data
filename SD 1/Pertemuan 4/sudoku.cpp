...
bool solveSudoku(vector<vector<int>> &board)
{
    ...
    // Jika tidak ada sel kosong, Sudoku sudah selesai
    if (!emptyFound)
        return true;
    ...
    for (int num = 1; num <= 9; num++)
    {
        if (isValid(board, row, col, num))
        {
            ...
            // Jika gagal, kosongkan lagi (backtrack)
            board[row][col] = 0;
        }
    }

    // Jika semua angka gagal, kembali ke langkah sebelumnya
    return false;
}
